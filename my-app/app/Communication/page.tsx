...
export default function VideoChat() {
  ...
  const stopRecording = async () => {
    if (mediaRecorderRef.current && isRecording) {
      ...
      const { user_audio, response } = await fetchVideoAnalysis();
  
      setMessages((prev) => [
        ...prev,
        { type: "user", content: user_audio }, // Display user input from backend
        { type: "bot", content: response }, // Display bot response
      ]);
